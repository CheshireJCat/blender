...
allowed-tools: Read Bash Glob Grep blender_python blender_scene_info blender_scene_info blender_render
---
...
Build code in **small, self-contained chunks** (each `blender_python` call gets a fresh Python namespace; only `bpy.data` persists between calls). Always re-import what you need.
...
1. `blender_scene_info` — confirm expected objects exist.
2. For paired parts (left/right lens), compare the returned object dimensions. Tolerance: 1 mm.
3. Triangle count: use the returned mesh polygon counts. If a part exceeds budget, plan Decimate.
...
|---------|-------------|-----|
| Python traceback from `blender_python` | Bad Python in generated code | Re-emit code in smaller chunks; trace the line from the error message |
| `blender_status.available` is false | Blender executable unavailable | Report the executable/configuration error |
| Blender timeout | Code chunk too large or slow | Break it into smaller `blender_python` calls |
